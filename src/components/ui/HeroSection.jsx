...
//             <Link
//               to="/listing"
//               className="px-6 py-1.5 cursor-pointer rounded-md border-1 border-white text-gray-400"
//             >
//               Shop New
//             </Link>
//             <Link
...
const HeroSection = () => {
  ...
  const handleConditionClick = (condition) => {
    if (searchTerm.trim()) {
      navigate(
        `/listing?query=${encodeURIComponent(
          searchTerm.trim()
        )}&condition=${condition}`
      );
    } else {
      navigate(`/listing?condition=${condition}`);
    }
  };
