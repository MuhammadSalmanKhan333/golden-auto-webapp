...
const BestSellingSection = ({ cars, loading, error }) => {
  return (
    <div className="rounded-lg mt-14 mb-10">
      ...
      {loading ? (
        ...
        <div className="bg-gray-400 border-x-4 border-red-500 text-red-700 p-4 mb-6">
          <p className="font-bold">Error loading best selling cars</p>
          <p>{error}</p>
          <button
        ...
        <div className="text-center py-10 text-lg text-gray-300">
          No best selling cars available at the moment
        </div>
      )}
    </div>
  );
